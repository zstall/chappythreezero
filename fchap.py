...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            g.user = usr.user_id
            u = usr.user_id
            return redirect(url_for('chores', usr=u, _external=True))
        except:
...
def chores(usr):
    ...
    if usr.admin == True and usr.super_user == False:
...
def run_chappy():
    ...
    for org_id in org_ids:
        query_org_users = ("SELECT user_id FROM users WHERE '" + org_id[0] +"' = ANY(org_ids) and user_super_user = False;")
